...
local FA_IntoxicationBadge = Class(Badge, function(self, owner)
	Badge._ctor(self, "fa_intoxicationbadge", owner)
...
function FA_IntoxicationBadge:SetPercent(val, max, penaltypercent)
...
function FA_IntoxicationBadge:OnUpdate(dt)
	...
	local anim = nil
	anim = "neutral"
	if rate > 0 and self.owner.components.fa_intoxication:GetPercent(true) < 1 then
	...
	elseif rate < 0 and self.owner.components.fa_intoxication:GetPercent(true) > 0 then
